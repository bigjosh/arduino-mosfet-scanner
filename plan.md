...
*Design rationale and v2 roadmap, carried over from the project's original
combined repo. Some narrative below refers to the earlier lab bench
(Siglent SPD3303C + Joulescope JS110) where this rig was prototyped — this
standalone repo contains only the Arduino scanner itself.*

## Context & goal
The repo currently characterizes MOSFETs with a **Siglent SPD3303C** PSU + **Joulescope JS110** driven by `scan_mosfet.py` over SCPI/VISA (CSV schema `Vds (V), Vgs (V), Ids (uA)`). [spec.md](spec.md) asks for a from-scratch **Arduino Uno** scanner that drives 3 pins and measures **Ids 0–1000 µA** and **Igs 0–100 µA** to **< 5 %**.
...
`host-tools/scan_arduino.py` (pyserial + numpy + matplotlib):
